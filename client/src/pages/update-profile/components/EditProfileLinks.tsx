...
const EditProfileLinks = ({links, setLinks}: Props) => {
  ...
  const handleClose = (removedLink: string) => {
    const newLinks =
      links && links.length > 0 ? links.filter((link) => link !== removedLink) : []
    setLinks(newLinks)
  }
  ...
  const handleInputConfirm = () => {
    if (inputValue && links && links.length > 0 && links.indexOf(inputValue) === -1) {
  ...
  const handleEditInputConfirm = () => {
    let newLinks = links && links.length > 0 ? [...links] : []
